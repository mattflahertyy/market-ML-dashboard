...
async def replay_ticks(symbol="AAPL", interval="1m", total_duration=200.0):
    """
    Replay ticks for today's session if available,
    otherwise fall back to the most recent trading day.
    """
    today = dt.date.today()

    # Helper: get most recent weekday (Mon–Fri)
    ...
    # Try to get today's (or last weekday’s) data
    df = yf.download(
        tickers=symbol,
